        ...
        public User FindByPrimaryEmail(string username)
        {
            using (var db = new DatabaseContext(DatabaseSelect.MatchGuide))
            {
                const string query = @"SELECT User_Login.UserId Id, 
                                        ...
                                        users.[UserType], 
                                        users.[ClientPortalTypeID] ClientPortalType,
                                        users.ClientPortalFTAlumniTypeID FloThruAlumniAccess,
                                        User_Login.[Login],
                                        User_Login.[Password] PasswordHash,
                                        C.[CompanyID] CompanyId,
                                        C.[CompanyName],
                                        C.[MaxVisibleRatePerHour] as ClientsMaxVisibleRate,
                                        C.[IsHavingFTAlumni] IsCompanyParticipating 
                                        From     user_email 
                                        INNER JOIN users ON users.userid = user_email.userid 
                                        INNER JOIN [User_Login] ON [User_Login].userid = users.userid 
                                        LEFT JOIN [Company] as C on users.CompanyID = C.CompanyID 
                                        Where user_email.[PrimaryEmail] = @Username 
                                        and User_Login.ForceUpdate = 0 
                                        and users.usertype in (select picklistid from dbo.udf_getpicklistids('userroles','Candidate,New Candidate',-1))";
